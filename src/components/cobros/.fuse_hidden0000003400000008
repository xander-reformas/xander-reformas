...
export default function Cobros() {
  ...
  return (
    <div className="p-6 max-w-5xl">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-navy">Cobros</h1>
        <p className="text-sm text-stone mt-0.5">Control de facturas pendientes y pagadas</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="card border-l-4 border-l-gold">
          ...
          <div className="text-xs text-stone mt-1">{pendientes.length} factura{pendientes.length !== 1 ? 's' : ''}</div>
        </div>
        <div className="card border-l-4 border-l-green-500">
          <div className="text-xs font-semibold uppercase tracking-wide text-stone mb-1">Total cobrado</div>
          <div className="text-2xl font-bold text-green-700">
            {totalCobrado.toLocaleString('es-ES', { style: 'currency', currency: 'EUR' })}
          </div>
          <div className="text-xs text-stone mt-1">{cobradas.length} factura{cobradas.length !== 1 ? 's' : ''} pagada{cobradas.length !== 1 ? 's' : ''}</div>
        </div>
        <div className={`card border-l-4 ${vencidasSinCobrar.length > 0 ? 'border-l-red-500' : 'border-l-stone/30'}`}>
          <div className="text-xs font-semibold uppercase tracking-wide text-stone mb-1">Vencidas sin cobrar</div>
          <div className={`text-2xl font-bold ${vencidasSinCobrar.length > 0 ? 'text-red-600' : 'text-stone'}`}>
      ...
      {hayAlertas && (
        <div className="card mb-6 border border-red-200">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <div className="font-bold text-navy text-sm">Alertas de cobro</div>
              <div className="text-xs text-stone">{porNivel.rojo.length + porNivel.naranja.length} factura{porNivel.rojo.length+porNivel.naranja.length!==1?'s':''} urgente{porNivel.rojo.length+porNivel.naranja.length!==1?'s':''}</div>
            </div>
          </div>
          ...
          <div className="flex gap-2 mb-4">
            {['rojo','naranja','amarillo','verde'].map(nivel => {
              ...
              return (
                <div key={nivel} className={`flex-1 rounded-xl border px-3 py-2 text-center ${count>0?cfg.bg:'bg-arena/30 border-arena-dark'}`}>
                  <div className={`w-3 h-3 rounded-full mx-auto mb-1 ${count>0?cfg.color:'bg-stone/20'}`} />
                  <div className={`text-lg font-black ${count>0?cfg.text:'text-stone/40'}`}>{count}</div>
                  <div className="text-[10px] text-stone leading-tight">{cfg.label}</div>
                </div>
              )
            })}
          </div>
            ...
            return (
              <div key={f.id} className={`flex items-center gap-3 rounded-xl border px-3 py-2 mb-2 ${cfg.bg}`}>
                ...
                <div className="flex-1 min-w-0">
                  <span className="font-semibold text-navy text-sm">{f.numero}</span>
                  {f.clientes?.nombre && <span className="text-xs text-stone ml-2">{f.clientes.nombre}</span>}
                </div>
                <div className="text-right flex-shrink-0">
                  <div className="font-bold text-navy text-sm">{totalFactura(f).toLocaleString('es-ES',{style:'currency',currency:'EUR'})}</div>
      ...
      {meses.length > 0 && (
        <div className="card mb-6">
          <div className="text-xs font-bold uppercase tracking-widest text-stone mb-4">Cobros por mes</div>
            ...
            {meses.reverse().map(([mes, valor]) => {
              ...
              return (
                <div key={mes} className="flex-1 flex flex-col items-center gap-1">
                  ...
                  <div className="w-full bg-arena-dark rounded-t-md transition-all" style={{ height: `${Math.max(altura, 4)}%`, minHeight: '4px', background: '#C9A84C' }} />
      ...
      <div className="flex gap-1 bg-arena-dark rounded-xl p-1 w-fit mb-5">
        <button onClick={() => setTab('pendientes')}
          className={`px-5 py-2 text-sm font-semibold rounded-lg transition-colors ${tab === 'pendientes' ? 'bg-white text-navy shadow-sm' : 'text-stone hover:text-navy'}`}>
          Pendientes ({pendientes.length})
        </button>
        <button onClick={() => setTab('cobrados')}
          className={`px-5 py-2 text-sm font-semibold rounded-lg transition-colors ${tab === 'cobrados' ? 'bg-white text-navy shadow-sm' : 'text-stone hover:text-navy'}`}>
      ...
      {/* Lista */}
      {loading ? (
        <div className="text-stone text-sm py-10 text-center">Cargando…</div>
      ) : lista.length === 0 ? (
        <div className="card text-center py-14">
          <div className="text-5xl mb-3">{tab === 'pendientes' ? '✅' : '💳'}</div>
          <div className="font-bold text-navy mb-1">
              ...
              <div key={f.id} className={`card flex items-center gap-4 ${vencida ? 'border-red-200 bg-red-50/30' : urgente ? 'border-orange-200 bg-orange-50/30' : ''}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 flex-wrap">
                    <span className="font-bold text-navy">{f.numero}</span>
                    {f.clientes?.nombre && <span className="text-sm text-stone">{f.clientes.nombre}</span>}
                    {f.obras?.nombre && <span className="text-xs text-stone/60 hidden md:inline">· {f.obras.nombre}</span>}
                  </div>
                  <div className="flex flex-wrap gap-x-4 gap-y-0.5 mt-1 text-xs text-stone">
                ...
                <div className="text-right flex-shrink-0">
                  ...
                  {tab === 'pendientes' ? (
                    ...
                    <button onClick={() => desmarcarPagada(f.id)}
                      className="mt-1 text-xs text-stone/50 hover:text-stone transition-colors">
                      Desmarcar
                    </button>
                  )}
                </div>
        ...
        <div className="fixed inset-0 bg-navy/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
            <div className="px-6 py-4 border-b border-arena-dark flex items-center justify-between">
              <h2 className="text-lg font-bold text-navy">Confirmar cobro</h2>
              <button onClick={() => setShowModal(false)} className="text-stone hover:text-navy text-2xl leading-none">×</button>
            </div>
            <form onSubmit={confirmarCobro} className="p-6 space-y-4">
              <div className="bg-arena rounded-xl p-4 text-center">
                <div className="text-sm text-stone">{facturaActiva.numero} · {facturaActiva.clientes?.nombre}</div>
